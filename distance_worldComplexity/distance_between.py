	...
	def find_boxes(self, cnts, width, image):
		...
		for ref in boxesList:
			...
			for c in cnts:  # loop over the contours individually
				...
				cv2.circle(orig, (int(refCoords[4][0]), int(refCoords[4][1])), 3, (240, 0, 159), -1)
				cv2.circle(orig, (int(objCoords[4][0]), int(objCoords[4][1])), 3, (240, 0, 159), -1)
				cv2.line(orig, (int(refCoords[4][0]), int(refCoords[4][1])), (int(objCoords[4][0]), int(objCoords[4][1])),
						 (240, 0, 159), 1)
				if not refD == 0 :
